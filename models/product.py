...
class product_template(models.Model):
    ...
    @api.multi
    def action_meli_product_variants(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window'].browse(self.env.ref('meli_oerp.action_meli_product_variants').id).read()[0]
        ...
        #     # 'view_id': self.env.ref('meli_oerp.action_meli_product_variants').id,
        #     'view_id': self.env.ref('meli_oerp.view_meli_product_variants_tree').id,
            ...
            if (product.meli_pub_as_variant):
                ...
                for variant in product.product_variant_ids:
                    if (variant._conditions_ok() ):
                        ...
                        _logger.error("No condition met for:"+variant.display_name)
            else:
                for variant in product.product_variant_ids:
                    ...
                    if (variant.meli_pub):
                        ...
                        _logger.error("No meli_pub for:"+variant.display_name)
    ...
    def _variations(self):
        # Definir las imagenes aca adentro
        variations = False
    ...
    def _get_meli_category_from_predictor(self):
        ...
        if rjson and isinstance(rjson, list):
            if "id" in rjson[0]:
                ...
                if (meli_categ==None):
                    _logger.error("Import category failed.")
...
class product_product(models.Model):
    ...
    def _meli_set_product_price( self, product_template, meli_price ):
        ...
        if (product_template.taxes_id):
            tax_total = 0
            for tax_id in product_template.taxes_id:
                if (tax_id.type_tax_use=="sale"):
                    tax_total = tax_total + tax_id.amount
            if (tax_total>0):
                ml_price_converted = meli_price / (1.0 + tax_total*0.01)
    ...
    def _meli_set_category( self, product_template, category_id ):
        ...
        if (ml_cat_id):
            ...
            if www_cat_id:
                p_cat_id = self.env['mercadolibre.category'].search([('id','=',www_cat_id)])
    ...
    def _meli_set_images( self, product_template, pictures ):
        ...
        try:
            ...
            _logger.error("_meli_set_images Exception")
            _logger.error(e, exc_info=True)
    
    def _get_descriptions_from_response(self, meli, response_json=False):
        """
            Retorna una tupla de 2 elementos; el primero es description, el segundo es plain_description
        """
        #TODO: traer la descripcion: con
        #https://api.mercadolibre.com/items/{ITEM_ID}/description?access_token=$ACCESS_TOKEN
        description = ''
        plain_description = ''
        if response_json and response_json['descriptions']:
            response = meli.get("/items/"+self.meli_id+"/description", {'access_token':meli.access_token})
            rjson = response.json()
            if 'text' in rjson:
                description = rjson['text']
            if 'plain_text' in rjson:
                plain_description = rjson['plain_text']
            if (len(description)>0):
                plain_description = description
        return (description, plain_description)

    def _compute_variations_from_response(self, response_json=False):
        if ('variations' in response_json):
            ...
            index = -1
            for variation in response_json['variations']:
                index = index + 1
                if ('attribute_combinations' in variation):
                    _attcomb_str = ""
                    response_json['variations'][index]["default_code"] = ""
                    for attribute_combination in variation['attribute_combinations']:
                        name_capitalized = attribute_combination['name']
                        # Si hay un nombre para la combinación
                        if (len(name_capitalized)):
                            # se nombra un atributo?
                            att = {
                                'name': name_capitalized,
                                'value_name': attribute_combination['value_name'],
                                'create_variant': default_create_variant,
                                'att_id': False
                            }
                            # Si tiene un id (existe en la base?)
                            if ('id' in attribute_combination):
                                if (attribute_combination["id"]):
                                    if (len(attribute_combination["id"])):
                                        att["att_id"] = attribute_combination["id"]
                            if (att["att_id"]==False):
                                name_capitalized = name_capitalized.strip()
                                # Pone el nombre con la primera letra en mayusculas
                                name_capitalized = name_capitalized[0].upper()+name_capitalized[1:]
                                att["name"] = name_capitalized
                            # Que carajos hace create_variant ???
                            if ('create_variant' in attribute_combination):
                                att['create_variant'] = attribute_combination['create_variant']
                            else:
                                response_json['variations'][index]["default_code"] = response_json['variations'][index]["default_code"]+name_capitalized+":"+attribute_combination['value_name']+";"
                            if (att["att_id"]):
                                ...
                                #customizado
                                attribute = self.env['product.attribute'].search([('name','=',name_capitalized),('meli_default_id_attribute','=',False)])
                                if (attribute_combination['name']!=name_capitalized):
                                    attribute_duplicates = self.env['product.attribute'].search([('name','=',attribute_combination['name']),('meli_default_id_attribute','=',False)])
    ...
    def product_meli_get_product( self ):
        ...
        # En esta porcion se recupera el producto desde mercadolibre
        try:
            ...
            return {}
        description, plain_description = self._get_descriptions_from_response(meli, rjson)
        video = ''
        if 'error' in rjson:
            return {}
        ...
        #TODO: revisar este metodo -> _meli_set_category
        #categories
        ...
        meli_fields = {
            ...
            'meli_description': plain_description,
            ...
            'meli_video': str(video),
            'meli_dimensions': meli_dim_str,
        }

        tmpl_fields = {
          'name': meli_fields["name"],
          'description_sale': plain_description,
          #'name': str(rjson['id']),
        ...
        if (rjson['available_quantity']>=0):
            if (product_template.type not in ['product']):
                try:
                    ...
                    _logger.error("Set type almacenable ('product') not possible:")
                    _logger.error(e, exc_info=True)
                    pass
        ...
        self._compute_variations_from_response(response_json=rjson)
        ...
        #this write pull the trigger for create_variant_ids()... checkeado, es correcto
        ...
        if (published_att_variants):
            ...
            for variant in product_template.product_variant_ids:
                ...
                _variant_default_code = ""
                for att in att_value_ids(variant):
                    _variant_default_code = _variant_default_code + att.attribute_id.name+':'+att.name+';'
                for variation in rjson['variations']:
                    if (len(variation["default_code"]) and (variation["default_code"] in _variant_default_code)):
                        if ("seller_custom_field" in variation):
                            ...
                            variant.default_code = variant.meli_id+'-'+_variant_default_code
            ...
            if (len(rjson['attributes']) and 1==1):
                for att in rjson['attributes']:
                    try:
                        ...
                        _logger.error("Attributes exception:")
                        _logger.error(e, exc_info=True)
        return {}
    ...
    def product_meli_status_active( self ):
        ...
        for product in self:
            ...
            if not response:
                _logger.error("product_meli_status_active error")
    ...
    def product_meli_upload_multi_images(self):
        ...
        if variant_image_ids(product.product_tmpl_id)==None:
        ...
        for product_image in variant_image_ids(product.product_tmpl_id):
    ...
    def product_on_change_meli_banner(self):
        ...
        product = self
        banner = product.meli_description_banner_id
    ...
    def _combination(self):
        ...
        # Publica el precio de la variante principal si es que el precio no esta establecido
        if (product_tmpl.meli_pub_principal_variant.id and price==False):
            price = product_tmpl.meli_pub_principal_variant.meli_price

        #Publica la cantidad de la variante principal si es que la cantidad no esta establecida o es igual a cero <- MAAAL!
        # if (product_tmpl.meli_pub_principal_variant.id and (qty==False or qty==0)):
        # if (product_tmpl.meli_pub_principal_variant.id and qty==False):
        ...
        # Aca determinar la imagen segun el att asociado
        meli_image_ids = self.env['product.image'].search([('product_attribute_id', 'in', product.attribute_value_ids.mapped('id'))]).mapped('meli_imagen_id')
        if meli_image_ids:
            var_comb["picture_ids"] = meli_image_ids
    ...
    @api.multi
    def _dummy_is_product_combination(self, variation):
        return self._is_product_combination(variation)
    ...
    def _is_product_combination(self, variation ):
        ...
        if 'attribute_combinations' in _self_combinations:
            for att in _self_combinations['attribute_combinations']:
                _map_combinations[att["name"]] = att["value_name"].capitalize()
    ...
    # Este parece ser un metodo de prueba
    def _validate_category_settings( self, body ):
        return body
        for product in self:
            if (product.meli_category and str(product.meli_category.meli_father_category_id) in ["MLA122201","MLA1540"]):
                body["price"] = None
    ...
    def _set_product_template_company_defaults(self):
        self.ensure_one()
        product_tmpl = self.product_tmpl_id
        product = self
        company = self.env.user.company_id
        # Checkea de los valores por default de la compañía
        # Tipo de lista (oro, plata, bronce, etc)
    ...
    def _set_product_meli_fields(self):
        self.ensure_one()
        product_tmpl = self.product_tmpl_id
        product = self
        warningobj = self.env['warning']
    ...
    def _product_post(self):
        ...
        product._set_product_template_company_defaults()

        product._set_product_meli_fields()

        #Hacer lo mismo con los atributos

        # attributes = []
        
        attributes = product._get_attributes()
        variations_candidates = False
        if product_tmpl.attribute_line_ids:
            for at_line_id in product_tmpl.attribute_line_ids:
                atname = at_line_id.attribute_id.name ## Primero Talle despues Color
                #atributos, no variantes! solo con un valor... #No es el caso de casi ningun producto
                if (len(at_line_id.value_ids)==1):
                    ...
                    # Entra acá porque casi siempre hay mas de un valor por atributo
        ...
        multi_images_ids = {}
        # if (variant_image_ids(product)):
        if (variant_image_ids(product_tmpl)):
        ...
        # Si está chequeado publicar como variantes:
        if (product_tmpl.meli_pub_as_variant):
            if (product_tmpl.meli_pub_principal_variant.id):
                if (product_tmpl.meli_pub_principal_variant.id == product.id):
                    ...
                    # Si hay productjson, es porque estamos actualizando
                    ...
                    if ( productjson and len(productjson["variations"]) ):
                        ...
                        _updated_ids = [x.id for x in vars_updated]
                    ...
                    _logger.error("Variant not able to post, variant principal.")
                    return {}
            else:
                _logger.error("Variant principal not defined yet. Cannot post.")
                return {}
        else:
    ...
    def product_post_stock(self):
        ...
        try:
            ...
            # Publicar como variantes
            if (product_tmpl.meli_pub_as_variant):
                productjson = False
                # Si no tiene variante principal establecida, la busca en sus variantes; la primera
                ...
                # Con la variante principal:
                if (product_tmpl.meli_pub_principal_variant.id):
                    # Si hay meli_id, es decir si es un producto que ya esta subido:
                    base_meli_id = product_tmpl.meli_pub_principal_variant.meli_id
                    if (base_meli_id):
                        # Consulta en mercadolibre al producto
                        response = meli.get("/items/%s" % base_meli_id, {'access_token':meli.access_token})
                        if (response):
                            productjson = response.json()

                # Si es un producto que ya estaba subido, reviso si tiene variaciones/variantes
                #chequeamos la variacion de este producto
                if ( productjson and len(productjson["variations"]) ):
                    #Variable con un diccionario vacio conteniendo nuevas variaciones
                    varias = {
                        "variations": []
                    }
                    ...
                    # Fotos / Imagenes
                    pictures_v = []
                    # Boleano que indica si el precio concide
                    same_price = False
                    
                    #Itera sobre las variaciones que recibio desde mercadolibre
                    for ix in range(len(productjson["variations"]) ):
                        ...
                        if (product._is_product_combination(productjson["variations"][ix])):
                            found_comb = True
                            product.meli_id_variation = productjson["variations"][ix]["id"]
                            var = {
            ...
            #ELSE: Publicar como producto sin variantes
            else:
                response = meli.put("/items/"+product.meli_id, fields, {'access_token':meli.access_token})
                if (response.content):
                    rjson = response.json()
                    if not 'available_quantity' in rjson:
                        _logger.error( "Error posting stock: %s" % response.content)
            ...
            _logger.error("product_post_stock > exception error")
            _logger.error(e, exc_info=True)
            pass
